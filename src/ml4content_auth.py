...
def rf_classification(authentications_per_person, user_names, dates, n_estimators=100):
    # 生成示例数据
    num_people = len(user_names)
    labels = np.repeat(np.arange(num_people), authentications_per_person)
    data = data_processing(authentications_per_person=authentications_per_person, user_names=user_names, dates=dates,
...
def main():
    ...
    rf_classification(authentications_per_person=authentications_per_person, user_names=user_names, dates=dates)
