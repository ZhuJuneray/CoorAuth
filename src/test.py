...
def difference_gaze_head(user, date, num, eye='L', angle='Yaw', add_noise_flag=False, noise_level=0.1):
    if eye not in ['L', 'R']:
        raise ValueError("eye must be 'L' or 'R'")
    if angle not in ['Yaw', 'Pitch', 'Roll']:
        raise ValueError("angle must be 'Yaw' or 'Pitch' or 'Roll'")
    
    # Assume data is stored in 'data/unity_processed_data'
    gaze_data_path = os.path.join("data/unity_processed_data", f"GazeCalculate_data_{user}-{date}-{num}_unity_processed.csv")
    head_data_path = os.path.join("data/unity_processed_data", f"Head_data_{user}-{date}-{num}_unity_processed.csv")
    
    df1 = pd.read_csv(gaze_data_path)
    df2 = pd.read_csv(head_data_path)

    # Add noise if flagged
    if add_noise_flag:
        df1[eye + '_' + angle] = add_noise(df1[eye + '_' + angle], noise_level=noise_level)
        df2[angle] = add_noise(df2[angle], noise_level=noise_level)

    # Calculate initial yaw angle offsets for gaze and head
    gaze_zero = df1[eye + '_' + angle][0]
    head_zero = df2[angle][0]

    gaze_df = [x - gaze_zero for x in df1[eye + '_' + angle]]
    head_df = [x - head_zero for x in df2[angle]]

    # Return the difference between gaze and head yaw angles
...
def data_processing(authentications_per_person, user_names, dates, rotdir=None, model="", noise_flag=False, noise_level=0.1):
    # 特征
    ...
    for user in user_names:
        for num in range(authentications_per_person):
            for date in dates:
                ...
                if execute_block:
                    if os.path.exists(rotdir + "\Head_data_" + user + '-' + date + '-' + str(num + 1) + '.csv') == False:
                        continue
                ...
                if noise_flag:
                    data_head = add_noise(data_head, noise_level=noise_level)
                ...
                if noise_flag:
                    data_eye = add_noise(data_eye, noise_level=noise_level)
                data_eye = add_noise(data_eye, noise_level=noise_level)
                ...
                # Head and eye points
                diff_yaw_data = difference_gaze_head(user, date, num+1)
                diff_yaw_smooth = smooth_data(diff_yaw_data, window_parameter=3)
                dy_el_feat = extract_features(np.array(diff_yaw_smooth))
                diff_pitch_data = difference_gaze_head(user, date, num+1, eye='L', angle='Pitch')
                diff_pitch_smooth = smooth_data(diff_pitch_data, window_parameter=3)
                dp_el_feat = extract_features(np.array(diff_pitch_smooth))
                diff_roll_data = difference_gaze_head(user, date, num+1, eye='L', angle='Roll')
                diff_roll_smooth = smooth_data(diff_roll_data, window_parameter=3)
...
################################################################ knn 二分类
def knn4con_binary(authentications_per_person, user_names, dates, model, n_neighbors=3, positive_label=None,
                   latter_auth_per_person=0, latter_user_names=None, latter_dates=None, latter_positive_label=None,):
    # 生成示例数据
    ...
    data = data_processing(authentications_per_person=authentications_per_person, user_names=user_names, dates=dates,
                           rotdir=os.path.join(os.getcwd(), "data"), model=model)
...
################################################################ knn 多分类
def knn4con_multi(authentications_per_person, user_names, dates, model, n_neighbors=3):
    # 生成示例数据
...
def svm4con_multi(authentications_per_person, user_names, dates, model, kernel="linear", C=1, gamma=0.02,
                  latter_auth_per_person=0, latter_user_names=None, latter_dates=None):
...
################################################################ knn kfold 二分类
def knn4con_binary_kfolds(authentications_per_person, user_names, dates, model, n_neighbors=3, n_splits=5, positive_label=None):
    # 生成示例数据
    num_people = len(user_names)
    # labels = np.repeat(np.arange(num_people), authentications_per_person)
    labels = np.array([1 if user in positive_label else 0 for user in user_names for _ in range(authentications_per_person)])
...
################################################################ knn kfold 多分类
def knn4con_multi_kfolds(authentications_per_person, user_names, dates, model, n_neighbors=3, n_splits=5):
    # 生成示例数据
    num_people = len(user_names)
    labels = np.repeat(np.arange(num_people), authentications_per_person)
    # labels = np.array([0 if user == 'zs' else 1 for user in user_names for _ in range(authentications_per_person)])
...
################################################################ svm kfold 二分类
def svm4con_binary_kfolds(authentications_per_person, user_names, dates, model, kernel="linear", C=1, gamma = 0.02, n_splits=3, positive_label=None):
    labels = np.array([1 if user in positive_label else 0 for user in user_names for _ in range(authentications_per_person)])
    data = data_processing(authentications_per_person=authentications_per_person, user_names=user_names, dates=dates,
                           rotdir=os.path.join(os.getcwd(), "data"), model=model)
    scaler = StandardScaler()
...
################################################################ svm kfold 多分类
def svm4con_multi_kfolds(authentications_per_person, user_names, dates, model, kernel="linear", C=1, gamma = 0.02, n_splits=3):
    num_people = len(user_names)
    labels = np.repeat(np.arange(num_people), authentications_per_person)
    data = data_processing(authentications_per_person=authentications_per_person, user_names=user_names, dates=dates,
                           rotdir=os.path.join(os.getcwd(), "data"), model=model)
...
def main():
    ...
    positive_label = [user_names[0], user_names[1]]  # 正样本
