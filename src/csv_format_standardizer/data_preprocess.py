...
def extract_features(sequence, slice_num=10):  # 把序列切成slice_num段，每段取均值、最大值、最小值、方差，共40个特征，返回一个拼接的一维数组
    # 计算每个子序列的基本长度
    n = len(sequence)
    sub_seq_length = n // slice_num
    remainder = n % slice_num  # 剩余的元素数量
...
def difference_lr_euler_angle(user, date, num): # 读取用户特定日期和序号的视线数据，以3个list分别返回左右视线Yaw, Pitch, Roll角度的差异, num从1开始
    data = pd.read_csv(os.path.join("data/unity_processed_data", "GazeCalculate_data_" + user + "-" + date + "-" + str(num) + "_unity_processed.csv"))
    # Create DataFrame
