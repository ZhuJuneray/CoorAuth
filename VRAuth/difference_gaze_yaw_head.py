...
for user in user_names:
    ...
    for num in range(repeat_num):
        for date in dates:
            ...
            axs_2[num, 0].plot([x - y for x, y in zip(L_Yaw_df, Yaw_df)], label='L_Gaze_Yaw - Head_Yaw '+user+str(num+1)+'_'+date)
    ...
    diagram_num += 1
    fig_2.savefig(os.path.join("unity_processed_picture","Yaw:gaze-head['" + str(user) + "'].png"))
    plt.clf()
