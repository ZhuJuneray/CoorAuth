...
for user in user_names:
    for num in range(repeat_num):
        for date in dates:
            ...
            axs_2[0, 0].plot(df['L_Yaw'], label=user+str(num+1)+'_'+date)
            axs_2[0, 0].set_title('L_Yaw')
            axs_2[0, 1].plot([x  if x < 180 else x - 360 for x in df['L_Pitch']])
            axs_2[0, 1].set_title('L_Pitch')
            axs_2[0, 2].plot([x  if x < 180 else x - 360 for x in df['L_Roll']])
...
fig_2.savefig(os.path.join("unity_processed_picture",'Gaze_data_calculated_by_unity' + str(user_names) + '.png'))
# plt.grid(True)
plt.show()
